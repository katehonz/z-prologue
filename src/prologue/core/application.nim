...
# Менеджер событий завершения
type ShutdownManager* = ref object
  events*: seq[Event]

proc newShutdownManager*(): ShutdownManager =
  new(result)
  result.events = @[]

proc addEvent*(manager: ShutdownManager, event: Event) =
  manager.events.add(event)

proc execEvents*(manager: ShutdownManager) =
  for event in manager.events:
    execEvent(event)

# Глобальный менеджер событий завершения
var shutdownManager = newShutdownManager()
...
proc handleContext*(app: Prologue, ctx: Context) {.async, gcsafe.} =
  ...
  ## Улучшенная обработка ошибок с более структурированным подходом
  ctx.middlewares = app.middlewares
  logging.debug(fmt"{ctx.request.reqMethod} {ctx.request.url.path}")

  # Обработка запроса с улучшенной обработкой ошибок
  try:
    await switch(ctx)
    ...
    logging.debug fmt"Route error: {e.msg}"
  except HttpError as e:
    # Обработка HTTP ошибок с сохранением кода ошибки
    if ctx.response.code == Http200:  # Если код ответа не был установлен
      ctx.response.code = Http400
    logging.debug fmt"HTTP error: {e.msg}"
  except AbortError as e:
    # Обработка прерываний запроса
    logging.debug fmt"Request aborted: {e.msg}"
  except Exception as e:
    # Обработка непредвиденных ошибок с подробным логированием
    let errorMsg = $e.name & ": " & e.msg & "\n" & getStackTrace()
    logging.error errorMsg
    
    ctx.response.code = Http500
    
    # В режиме отладки показываем полное сообщение об ошибке
    if app.appDebug:
...
proc prepareRun(app: Prologue) =
  ...
  # Добавляем события завершения в менеджер
  for event in app.shutdown:
    shutdownManager.addEvent(event)

  if app.shutdown.len != 0:
